...
├── playbooks/
│   └── files/
│       ├── root_id_ed25519         # Generated by ssh_hardening
│       ├── root_id_ed25519.pub
│       └── *(other key files if needed)*
└── roles/
    ├── bootstrap/                  # Install python3, openssh-server
    ├── ssh_hardening/              # Generate root keypair and deploy sshd_config
...
[cloudkey:vars]
target_user=red
# Optional: Override SSH key paths if needed
# ssh_key_file=playbooks/files/root_id_ed25519
# ssh_pub_key_file=playbooks/files/root_id_ed25519.pub
...
### Root SSH Key Configuration (ssh_hardening role)
- **ssh_key_file**: Path to the private key (default: `{{ playbook_dir }}/ssh_keys/root_ed25519`)
- **ssh_pub_key_file**: Path to the public key (default: `{{ playbook_dir }}/ssh_keys/root_ed25519.pub`)
- **ssh_key_algorithm**: Key type (default: `ed25519`)
- **ssh_key_comment**: Key comment (default: `root@{{ inventory_hostname }}`)
### User SSH Key Configuration (users role)
- **public_key_file** and **private_key_file**: File paths to the SSH key files to be imported for the **target_user**
